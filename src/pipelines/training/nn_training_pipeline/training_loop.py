import torch
from src.pipelines.training.helpers import compute_metrics
...
def training_loop(config, model, train_loader, test_loader, optimizer, criterion, scheduler, device):
    ...
    for epoch in range(epochs):
        train_loss, train_acc, train_prec, train_rec, train_f1, train_r2, last_epoch_train_data = train_one_epoch(model, train_loader, optimizer, criterion, device)
        test_loss, test_acc, test_prec, test_rec, test_f1, test_r2, last_epoch_test_data = evaluate(model, test_loader, criterion, device)
...
def train_one_epoch(model, train_loader, optimizer, criterion, device):
    ...
    acc, prec, rec, f1_micro, r2 = compute_metrics(all_labels, all_preds)
